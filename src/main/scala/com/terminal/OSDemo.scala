...
object OSDemo {
  def main(args: Array[String]): Unit = {
    ...
    //使用 sparkSQL处理数据
        val res1: DataFrame =dataFrame.select("client","requestmode","processnode","iseffective","isbilling",
          "isbid","iswin","adorderid","winprice","adpayment")

        res1.createOrReplaceTempView("sum_location")

        val res2=sparkSession.sql("select case client when 1 then 'an' when 2 then 'ios'  when 3 then'wp' else 'other' end ostype," +
          ...
          "from sum_location group by ostype")
        res2.show()

    //    使用spark-core 处理数据
//    dataFrame.rdd.map(line=>{
//      val client=line.getAs[Int]("client")
...
//
//      val ostype: String = PCtUtils.osType(client)
//

...
object TermiDemo {
  def main(args: Array[String]): Unit = {
    ...
    //使用 sparkSQL处理数据
    //    val res1: DataFrame =dataFrame.select("devicetype","requestmode","processnode","iseffective","isbilling",
    //      "isbid","iswin","adorderid","winprice","adpayment")
    //
    //    res1.createOrReplaceTempView("sum_location")
    //
    //    val res2=sparkSession.sql("select devicetype," +
    ...
    //      "from sum_location group by devicetype")
    //    res2.show()
    ...
    dataFrame.rdd.map(line=>{
      ...
      val devicetype=line.getAs[Int]("devicetype")
      ...
      val dev: String = PCtUtils.Devtype(devicetype)
